...
export default async function Page() {
  ...
  const { data: rows } = await supabase.from("locations").select("*").order("name");
  ...
  async function create(formData: FormData) {
    ...
    const { error } = await supabase.from("locations").insert({ name, active: true });
  ...
  return (
    <div className="max-w-4xl">
      <div className="text-2xl font-semibold">Admin — Locations</div>
      <Card className="mt-6">
