...
type GuestObj =
  | {
      ...
      email: any;
      phone: any;
      reservation_number: any;
      room_number: any;
    }
  | null;

type ClassObj =
  | { id: any; name: any; duration_minutes: any; base_price: any }
  | null;

type NameObj = { id?: any; name: any } | null;
...
export default async function Page({ params }: { params: { id: string } }) {
  ...
  const { data: r, error } = await supabase
    ...
    .select(
      ...
      requested_datetime,
      guest_message,
      guest:profiles(first_name,last_name,email,phone,reservation_number,room_number),
      cls:private_classes(id,name,duration_minutes,base_price),
      proposed_price,
      proposed_start_datetime,
      proposed_end_datetime,
      proposed_location_id,
      proposed_instructor_id,
      proposed_location:locations!private_requests_proposed_location_id_fkey(id,name),
      proposed_instructor:instructors!private_requests_proposed_instructor_id_fkey(id,name),
  ...
  const guest = firstOrSelf((r as any).guest) as GuestObj;
  const cls = firstOrSelf((r as any).cls) as ClassObj;

  const location = firstOrSelf((r as any).location) as NameObj;
  const instructor = firstOrSelf((r as any).instructor) as NameObj;

  const proposedLocation = firstOrSelf((r as any).proposed_location) as NameObj;
  const proposedInstructor = firstOrSelf((r as any).proposed_instructor) as NameObj;
  ...
  return (
    <div className="max-w-3xl">
      <div className="flex items-start justify-between gap-3">
        ...
        <Link href="/staff/programs/requests">
      ...
      <div className="mt-6 grid grid-cols-1 gap-4">
        <Card>
          ...
          <div className="mt-2 text-sm text-ink/70">
            ...
            <div>
              Phone: <span className="font-medium">{guest?.phone ?? "—"}</span>
            </div>
            <div>
              Reservation:{" "}
              <span className="font-medium">
        ...
        <Card>
          ...
          <div className="mt-1 text-sm text-ink/70">
            ...
            • Duration:{" "}
            <span className="font-medium">
              {cls?.duration_minutes != null ? `${cls.duration_minutes} min` : "—"}
            </span>{" "}
            • Base:{" "}
            <span className="font-medium">
              {cls?.base_price != null ? `$${cls.base_price}` : "—"}
            </span>
          </div>
        </Card>
        ...
        <Card>
          <div className="text-sm font-semibold">
            Counter-offer (Handshake required)
          </div>
          <div className="mt-2 text-xs text-ink/60">
            Create a counter-offer. Guest must accept or decline. If declined,
            status becomes <b>declined_by_guest</b> (final).
          </div>
          ...
          {canPropose ? (
            <form
              ...
              action={async (formData) => {
                ...
                const { error } = await supabase.rpc("leader_send_counter_offer", {
              ...
              {r.proposed_start_datetime ? (
                <>
                  ...
                  <div>
                    Proposed end:{" "}
                    <span className="font-medium">
                      {r.proposed_end_datetime
                        ? new Date(r.proposed_end_datetime).toLocaleString()
                        : "TBD"}
                    </span>
                  </div>
                  <div>
                  ...
                  <div>
                    Proposed price:{" "}
                    <span className="font-medium">
                      {r.proposed_price != null ? `$${r.proposed_price}` : "—"}
                    </span>
                  </div>
                </>
              ) : (
          ...
          {r.status === "approved" ? (
            <div className="mt-6 rounded-xl border border-border p-4">
              ...
              <div className="mt-1 text-sm text-ink/70">
                End:{" "}
                <span className="font-medium">
                  {r.approved_end_datetime
                    ? new Date(r.approved_end_datetime).toLocaleString()
                    : "TBD"}
                </span>
              </div>
              <div className="mt-1 text-sm text-ink/70">
              ...
              <div className="mt-1 text-sm text-ink/70">
                Location:{" "}
                <span className="font-medium">{location?.name ?? "TBD"}</span>
              </div>
              <div className="mt-1 text-sm text-ink/70">
                Price:{" "}
                <span className="font-medium">
                  {r.final_price != null ? `$${r.final_price}` : "TBD"}
                </span>
              </div>
            </div>
          ) : null}
